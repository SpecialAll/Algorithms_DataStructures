 ...
 * 那么我们从1开始乘以2,3,5，就得到2,3,5三个丑数，在从这三个丑数出发乘以2,3,5就得到4，6,10,6，9,15,10,15,25九个丑数，我们发现这种方***得到重复的丑数，
...
public class GetUglyNumber_Solution {
    public int GetUglyNumber_Solution(int index) {
        ...
        while(list.size() < index){
            //选出三个队列头最小的数
            newNum = Math.min(list.get(p1) * 2, Math.min(list.get(p2) * 3, list.get(p3) * 5));
            //这三个if有可能进入一个或者多个，进入多个是三个队列头最小的数有多个的情况
            if(list.get(p1)*2 == newNum) p1++;
            if(list.get(p2)*3 == newNum) p2++;
            if(list.get(p3)*5 == newNum) p3++;
            list.add(newNum);
        }
        return newNum;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.GetUglyNumber_Solution(21));
